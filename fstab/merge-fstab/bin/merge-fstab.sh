...
if dd if=/dev/block/by-name/system bs=256k count=1|strings|grep qti_dynamic_partitions > /dev/null; then
    ...
    for p in vendor cust; do
        echo "/dev/block/bootdevice/by-name/${p} /${p} ext4 ro,barrier=1 wait,formattable" >> /system/etc/recovery.fstab
    done
    echo "/dev/block/bootdevice/by-name/system /system_root ext4 ro,barrier=1 wait,formattable" >> /system/etc/recovery.fstab
